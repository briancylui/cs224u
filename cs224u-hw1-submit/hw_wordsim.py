...
def run_ppmi_lsa_pipeline(count_df, k):
    ...
    ##### YOUR CODE HERE
    count_df_ppmi = vsm.pmi(count_df)
    count_df_ppmi_svd = vsm.lsa(count_df_ppmi, k)
    return full_word_similarity_evaluation(count_df_ppmi_svd)
...
def run_small_glove_evals():
    ...
    for iterations in [10, 100, 200]:
        glove_model = GloVe(max_iter=iterations)
        ...
        results[iterations] = eval_results['Macro-average']
        print('max_iter {}: {:.3f}'.format(iterations, results[iterations]))
    return results
...
def dice(u, v):
    ...
    ##### YOUR CODE HERE
    fraction = 2 * np.sum(np.minimum(u, v)) / np.sum(u + v)
    return 1 - fraction
...
def ttest(df):
    ...
    return (P - expected) / np.sqrt(expected)
...
def subword_enrichment(df, n=4):
    ...
    for word in df.index:
        df_char_rep[word] = vsm.character_level_rep(word, df_char, n)
    ...
    ##### YOUR CODE HERE
    result = pd.DataFrame(df_char_rep).T
...
# 
# Code description:
# 1. Use the IMDB movie review dataset with window size 5 and scaled count weighting.
# 2. Re-weigh the 5000 x 5000 co-occurrence word matrix with PPMI
# 3. For each word, concatenate to its existing representation its normalized character-level representation based on n-grams with n = 4
# 4. Re-weight the extended (5000 x 10000) co-occurrence word matrix with t-test reweighting
# 5. Do retrofitting with edges based on WordNet
# 6. Length-normalize all word representations
# 7. Use an autoencoder to obtain a more concise 1000-dimensional (down from 10000-dim) latent representation for each word, with 200 iterations and a learning rate of 0.001
# 8. Evaluate and print the macro-average correlation over all 5 data sets on the latent output of the autoencoder.
# 
# My peak score was: 0.5774123805295883
...
if 'IS_GRADESCOPE_ENV' not in os.environ:
    # Special remark to graders: Please include the Python file 
    # retrofitting.py from the CS 224U repository for the purpose of 
    # grading the original system. At the time of HW1 submission, the
    # the autograder did not work whenever I submit a zip file, but
    # did work when I submitted just this .py file.  Thank you.
...
if 'IS_GRADESCOPE_ENV' not in os.environ:
    def retrofit(X_vsm, df, edges=None):
        if edges is None:
            edges = get_wordnet_edges()
        index_edges = convert_edges_to_indices(edges, X_vsm)
        retro = Retrofitter(verbose=True)
        X_retro = retro.fit(X_vsm, index_edges)
        return pd.DataFrame(X_retro, index=df.index)
...
if 'IS_GRADESCOPE_ENV' not in os.environ:
    def subword_enrichment_concatenate(df, n=4):
        ...
        # 2. Use `vsm.character_level_rep` to get the normalized representation
        # for every word in `df` according to the character-level
        # VSM you created above, and concatenate it to each existing representation for the corresponding word

        char_weight = 1.0
        df_char_rep = {}
        for word in df.index:
            df_char_rep[word] = df.loc[word].values
            char_rep = vsm.character_level_rep(word, df_char, n)
            char_rep_norm = vsm.vector_length(char_rep)
            if char_rep_norm > 0:
                char_rep /= char_rep_norm
            df_char_rep[word] = np.concatenate((df_char_rep[word], char_weight * char_rep), axis=None)

        # 3. Return a `pd.DataFrame` with the same index
        # values as `df`, but filled with the new representations
        # created at step 2.
...
if 'IS_GRADESCOPE_ENV' not in os.environ:
    def run_original_evals(data):
        data = vsm.pmi(data)
        data = subword_enrichment_concatenate(data, n=4)
        data = pd.DataFrame(ttest(data), index=data.index)
        data = retrofit(data, data)
        data = data.apply(vsm.length_norm, axis=1)
        data = TorchAutoencoder(
            max_iter=200, hidden_dim=1000, eta=0.001).fit(data)
        result = full_word_similarity_evaluation(data)
        print(result['Macro-average'])
        return result
